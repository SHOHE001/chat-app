...
- `admin`は`member`を、`owner`は`admin`と`member`をBAN・解除できる。本人と`owner`はBANできない。
...
- `owner`と`admin`は、自分のアカウント欄にある「▦」ボタンから登録用QRコードを生成できる。`member`にはこの操作を表示せず、生成APIも403で拒否する。
- QRコードには現在開いているChat Labと同じオリジンの`/?register=1`を格納する。読み取った端末では、保存済みセッションがあっても最初にアカウント登録画面を表示する。
...
- 現段階のQRは一回限りの招待券ではなく、既存の公開登録画面を開くための導線である。招待期限、利用回数、登録停止は未実装。
