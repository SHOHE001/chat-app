...
- `owner` はメンバー一覧から他アカウントを `admin` または4種類の一般ロールに変更できる。
...
- チャンネル作成時に`member`、`adult`、`child`、`staff`から入室可能ロールを複数選択できる。未選択なら全員向けになり、`owner`と`admin`は指定にかかわらず常に入室・管理できる。
- 限定チャンネルは権限のない利用者の一覧に表示せず、ルームIDを直接指定した入室も拒否する。入室中にロールを失った場合は自動的に「全体」へ戻す。Web Pushも入室可能な利用者だけへ配送する。
