...
アカウント、複数チャンネル、独立スレッド、メッセージ返信、メンション、リアクション、横断検索に対応した、ビルドレスのリアルタイムチャット。
...
- `adult`（大人）、`child`（子供）、`staff`（スタッフ）は権限ではなく複数選択可能な属性ロール。`owner`はmemberにもadminにも任意の組み合わせで付与でき、属性を付けても管理権限は変化しない。`owner`自身には属性ロールを付けず、単独の`owner`ロールが全管理権限と全チャンネルへのアクセスを内包する。
...
- 自分・メンバー・投稿のアイコンからプロフィールを開ける。属性ロールはメンバー一覧へ直接並べず、ownerが対象プロフィール内で変更する。
- リアクションは通常タップで付与・解除し、長押しすると付けたメンバー一覧を表示する。
...
- 画面上部の検索ボタンから2〜100文字で検索する。参加できる全チャンネルの通常投稿と独立スレッド投稿を横断し、新しい順に最大50件表示する。
- 結果にはチャンネル、スレッド名、投稿者、日時、本文を表示する。選ぶと対象チャンネルまたはスレッドを開き、直近履歴内にあれば投稿位置まで移動する。
- 入室権限のない限定チャンネル、削除済み・非表示の投稿は検索結果へ含めない。検索語のパーセントとアンダースコアはワイルドカードではなく通常文字として扱う。
