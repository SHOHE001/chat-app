...
test('P02b profile roles: 属性は権限と独立し、ownerは属性を持たず全権限を内包する', async () => {
  const ctx = await startServer();
  try {
    const owner = createClient(ctx.url);
    ...
    assert.deepEqual(ownerAuth.user.profile_roles, []);
...
test('P12 restricted rooms: 属性ロールで入室を判定し、ownerは属性なしでも常に入室できる', async () => {
  const ctx = await startServer();
  try {
    const owner = createClient(ctx.url);
...
test('P13 room notifications: アカウントごとにオンオフし、再ログイン後も設定を維持する', async () => {
  ...
  try {
    ...
    assert.equal(defaultRoom.notificationsEnabled, false);
...
test('P14 announcements: 全員が閲覧でき、owner/adminだけが投稿でき、削除できない', async () => {
  const ctx = await startServer();
  try {
    const owner = createClient(ctx.url);
    const member = createClient(ctx.url);
    await Promise.all([owner.open(), member.open()]);
    const ownerAuth = await register(owner, 'announcement-owner');
    await register(
      member,
      'announcement-member',
      'password-123',
    ...
    const ownerStatePromise = owner.next('state');
    owner.send('get_state');
    const ownerState = await ownerStatePromise;
    ...
    await memberSwitched;

    for (const [type, payload] of [
      ['message', { body: '一般ユーザーからのお知らせ' }],
      ['message', { body: '一般ユーザーの直接返信', replyToId: 999 }],
      ['create_thread', { title: '一般ユーザーのスレッド' }],
    ]) {
      const rejected = member.next('error');
      member.send(type, payload);
      assert.equal((await rejected).reason, 'announcement_read_only');
    }

    const ownerSwitched = owner.next('room_switched');
    owner.send('switch_room', { roomId: announcement.id });
    await ownerSwitched;
    const memberDelivery = member.next('message');
    const ownerEcho = owner.next('message');
    owner.send('message', { body: 'バージョン1.1を公開しました' });
    assert.equal((await memberDelivery).message.body, 'バージョン1.1を公開しました');
    assert.equal((await ownerEcho).message.body, 'バージョン1.1を公開しました');

    const cannotDelete = owner.next('error');
    owner.send('delete_room', { roomId: announcement.id });
    assert.equal((await cannotDelete).reason, 'cannot_delete_announcement');
    assert.equal(
      ctx.app.db.prepare('SELECT kind FROM rooms WHERE id = ?').get(announcement.id).kind,
      'announcement',
    );
...
test('P15 message search: 閲覧可能な通常投稿と専用スレッドだけを検索する', async () => {
  const ctx = await startServer();
  try {
    const owner = createClient(ctx.url);
    const member = createClient(ctx.url);
    await Promise.all([owner.open(), member.open()]);
    const ownerAuth = await register(owner, 'search-owner');
    await register(member, 'search-member', 'password-123', await issueInvite(ctx, ownerAuth.sessionToken));

    const ownerStatePromise = owner.next('state');
    owner.send('get_state');
    const defaultRoom = (await ownerStatePromise).rooms[0];

    const normalEcho = owner.next('message');
    owner.send('message', { body: '共通検索語 normal' });
    const normal = (await normalEcho).message;

    const createdThread = owner.next('thread_created');
    owner.send('create_thread', { title: '検索対象スレッド', body: '共通検索語 thread' });
    const thread = (await createdThread).thread;

    const hiddenEcho = owner.next('message');
    owner.send('message', { body: '共通検索語 hidden' });
    const hidden = (await hiddenEcho).message;
    const hiddenEvent = owner.next('message_hidden');
    owner.send('hide_message', { messageId: hidden.id });
    await hiddenEvent;

    const ownerRooms = owner.next('rooms');
    owner.send('create_room', { name: '検索不可チャンネル', allowedRoles: ['adult'] });
    const restricted = (await ownerRooms).rooms.find((room) => room.name === '検索不可チャンネル');
    const switched = owner.next('room_switched');
    owner.send('switch_room', { roomId: restricted.id });
    await switched;
    const secretEcho = owner.next('message');
    owner.send('message', { body: '共通検索語 secret' });
    await secretEcho;

    const resultsPromise = member.next('search_results');
    member.send('search_messages', { query: '共通検索語' });
    const response = await resultsPromise;
    assert.equal(response.query, '共通検索語');
    assert.equal(response.results.length, 2);
    assert.deepEqual(new Set(response.results.map((result) => result.kind)), new Set(['message', 'thread']));
    assert.ok(response.results.some((result) => result.messageId === normal.id && result.roomId === defaultRoom.id));
    assert.ok(response.results.some((result) => result.threadId === thread.id));
    assert.equal(response.results.some((result) => result.body.includes('hidden')), false);
    assert.equal(response.results.some((result) => result.body.includes('secret')), false);

    const invalidPromise = member.next('error');
    member.send('search_messages', { query: 'x' });
    assert.equal((await invalidPromise).reason, 'bad_search_query');
