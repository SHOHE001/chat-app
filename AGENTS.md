...
`chat-app` は、アカウント・チャンネル・独立スレッド・メッセージ返信・メンション・リアクション・横断検索を試すリアルタイムチャットのプロトタイプである。
...
- アプリ内の権限は `owner` / `admin` / `member` の3段階だけとする。`adult` / `child` / `staff` は権限階層ではなく、どの権限のアカウントにも0個以上付与できる属性ロールとして別管理する。旧 `ADMIN_PASSWORD` と `admin_auth` は使わない。
- 最初の登録者はowner。ownerだけが他アカウントの権限と属性ロールを変更でき、owner/adminがチャンネルを管理できる。owner自身には`adult` / `child` / `staff`を付与・表示せず、単独のownerロールが全管理権限と全チャンネルへのアクセスを内包する。属性ロールの変更UIは対象ユーザーのプロフィール内だけに表示する。
- owner/adminはチャンネル作成時に属性ロールの入室許可リストを設定できる。未選択は全員向け。ownerは常に入室できるが、adminは付与された属性で判定する。対象外利用者には一覧・直接入室・Web Pushのいずれからも内容を渡さない。作成後の許可リスト変更は未実装。
- 固定の「お知らせ」チャンネルは全員が閲覧でき、owner/adminだけが通常投稿・直接返信・スレッド作成・返信を行える。member権限は閲覧・リアクション・通報・通知設定のみ利用でき、チャンネル自体は削除できない。通知は「お知らせ」だけ既定ONで、利用者別のOFFをDBへ保存する。
- 認証済み利用者はチャンネルごとに通知をオン／オフできる。既定はオフ。オン設定はアカウント単位でSQLiteへ保存し、そのチャンネルの通常投稿・スレッド作成・返信だけをWeb Push対象にする。
- 認証済み利用者は、入室可能な全チャンネルの通常投稿と独立スレッド投稿を2〜100文字で横断検索できる。新しい順に最大50件を返し、非表示投稿と入室不可チャンネルはSQL検索時点で除外する。
- member権限は自分のメッセージだけを編集・削除できる。owner/adminは通常チャンネル・独立スレッドの全投稿を操作できる。UIだけでなくWebSocket側で権限を強制する。
- 認証済み利用者は全投稿を通報できる。対象投稿と前後2件ずつを通報時点で保全し、owner/adminだけが通報一覧を閲覧する。同一利用者・同一投稿の重複通報は禁止する。
- adminはmember権限を、ownerはadmin/member権限をBAN・解除できる。属性ロールはBAN階層へ影響しない。本人とownerはBAN不可。期間は10分、1時間、24時間、7日、30日、永久で、BAN時は全セッション・通知購読を削除して接続中WebSocketも切断する。
...
- 変更中のローカル検証は影響する挙動へ絞る。コード、設定、依存関係を変更した場合は関連テストを実行し、
  文書だけの変更では参照先と差分を確認する。merge前の必須CIでは `npm test` の全件成功を確認する。
...
- 標準配置への通常反映は、ランタイムへ影響する変更をPRでmergeし、既存の本番反映の承認範囲内で
  `sudo deploy/release.sh /home/shohei/プロジェクト/chat-app`を使い、merge済みcommit単位のreleaseを作って行う。
  文書やGitHub運用だけの変更は本番へ反映しない。
...
- 依存を必要最小限にした単一プロセスのプロトタイプ方針を維持し、Rust版の要件が固まる前にNode版を過剰に拡張しない。

## GitとGitHubの必須運用

- 最新のdefault branchから作業branchを作り、作業branchへpushしてPRを開く。default branchへ直接pushしない。
- 意味のある変更単位でcommitする。commit前に差分とstage対象を確認し、実資格情報、`.env`、DB、アップロード本体、
  セッショントークン、無関係な既存差分を含めない。
- `.githooks/pre-commit`と`.githooks/pre-push`を回避する`--no-verify`は使わない。clone後は`git config core.hooksPath .githooks`を設定する。
- PRの現在のhead SHAに対する必須CIが成功し、未解決レビューがないことを確認してから通常mergeする。
  admin bypass、force push、履歴書き換え、保護設定の一時的な弱体化は行わない。
- merge後の本番反映は上記の承認範囲と個別の運用制約に従う。PR作成、CI確認、mergeと本番反映を同じ判断として扱わない。
- 最終回答ではbranch、commit、PR、検証、CI、merge、本番反映の実状態を区別して報告する。
...
- PC・スマホでのメッセージ横断検索、限定チャンネルの非表示、検索結果からチャンネル／スレッドへの移動
- メッセージの日時、連投グルーピング、直接返信、メンション、リアクション、リアクション長押しでの付与者確認
