...
- 通報内容や会話を外部AIへ送る処理、定期AI巡回、費用が発生するジョブは通常反映に含めない。送信範囲・モデル・費用上限・実行間隔・人間による最終判断を決めてから、Webサーバーと分離したワーカーとして明示的に有効化する。
...
メンション限定・チャンネル別通知、パスワード変更・再発行、レート制限、通報の対応完了ワークフローやAI巡回は未実装であり、
